// 03_gemm.cu  -- a BIG, real workload: tiled single-precision matrix multiply C = A*B
//
// This is THE classic place cp.async earns its keep. Both kernels stage tiles of A and B
// into shared memory and do the same math; they differ ONLY in HOW the tile gets to shared:
//
//   gemm_sync  : As[ty][tx] = A[...];  -> LDG (global->register) + STS (register->shared)
//   gemm_async : __pipeline_memcpy_async(...) -> LDGSTS (global->shared, registers bypassed),
//                DOUBLE-BUFFERED so the NEXT tile streams in while the CURRENT tile is being
//                multiplied. That overlap is the whole point.
//
...
constexpr int TILE = 32;     // 32x32 tile -> 1024 threads/block (one C element per thread)
...
__global__ void gemm_sync(const float* __restrict__ A, const float* __restrict__ B,
    ...
    for(int m=0;m<numTiles;m++){
        As[ty][tx] = A[row*n + (m*TILE + tx)];      // LDG + STS  (staged via register)
...
__global__ void gemm_async(const float* __restrict__ A, const float* __restrict__ B,
                           float* __restrict__ C, int n){
    ...
    for(int m=0;m<numTiles;m++){
        ...
        if(m+1 < numTiles){                                   // issue NEXT tile's copy now...
            int mm = m+1;
            __pipeline_memcpy_async(&As[nxt][ty][tx], &A[row*n + (mm*TILE + tx)], sizeof(float));
            __pipeline_memcpy_async(&Bs[nxt][ty][tx], &B[(mm*TILE + ty)*n + col], sizeof(float));
            __pipeline_commit();
        }
        __pipeline_wait_prior(m+1<numTiles ? 1 : 0);          // ...wait only for CURRENT tile
        __syncthreads();
        #pragma unroll
        for(int k=0;k<TILE;k++) acc += As[cur][ty][k]*Bs[cur][k][tx]; // compute || next copy
        __syncthreads();
    }
    C[row*n + col] = acc;
}
...
int main(){
    ...
    CK(cudaMalloc(&Cs,bytes)); CK(cudaMalloc(&Ca,bytes));
    // fill with a known pattern via a tiny init kernel substitute: memset won't give floats,
    // so use host fill then copy.
    float* h = (float*)malloc(bytes);
    for(size_t i=0;i<(size_t)N*N;i++) h[i] = (float)((i%13)-6)*0.1f;
    ...
    double maxdiff=0; for(size_t i=0;i<(size_t)N*N;i++){ double d=fabs(hs[i]-ha[i]); if(d>maxdiff)maxdiff=d; }
    ...
    free(h); free(hs); free(ha);
    CK(cudaFree(A)); CK(cudaFree(B)); CK(cudaFree(Cs)); CK(cudaFree(Ca));
    return 0;
}
